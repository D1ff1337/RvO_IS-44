...
    void Update()
    ...
    {
        // Проверка, что это меч врага и враг в атакующем состоянии
        if (other.gameObject.CompareTag("EnemySword") && !isBlock && enemyController.isAttack && Time.time > lastHitTime + ignoreHitTime)
        {
            // Получаем текущую анимацию врага
            AnimatorStateInfo currentState = enemyController.animator.GetCurrentAnimatorStateInfo(0);

            // Проверяем, что враг не проигрывает анимацию KnockdownFront1
            
                lastHitTime = Time.time;
                playerHealth--;  // Уменьшаем здоровье игрока
                hpSlider.value = playerHealth;
                animator.Play("Sword_Hit_L_2");  // Воспроизводим анимацию попадания

                // Проверяем, если здоровье игрока равно нулю, то проигрыш
                if (playerHealth <= 0)
                {
